...
namespace LabWork21
{
    partial class MainViewModel : ViewModelBase
    {
        ...
        public List<string> GetInstalledApplicationName()
        {
            ...
            foreach (var subKeyName in openUserKey.GetSubKeyNames())
            {
                using (RegistryKey? subKey = openUserKey.OpenSubKey(subKeyName))
                {
            ...
            foreach (var subKeyName in openMachineKey.GetSubKeyNames())
            {
                using (RegistryKey? subKey = openMachineKey.OpenSubKey(subKeyName))
                {
                    if (subKey?.GetValue("DisplayName") != null)
                        InstalledApplicationsList.Add(subKey.GetValue("DisplayName").ToString());
                }
